...
// Lấy thông tin bác sĩ từ bảng users với role = doctor
...
$stmt = $db->prepare("SELECT COUNT(*) FROM appointments WHERE user_id = ? AND DATE(appointment_date) = ?");
$stmt->execute([$doctor_id, $today]);
$today_appointments = $stmt->fetchColumn();

$stmt = $db->prepare("SELECT COUNT(*) FROM appointments WHERE user_id = ? AND status = 'pending'");
$stmt->execute([$doctor_id]);
...
$stmt = $db->prepare("SELECT a.*, u.name as patient_name 
                    ...
                    WHERE a.user_id = ? AND u.role = 'user'
...
<div id="dashboard-page">
            ...
            <div class="content-wrapper flex-grow-1">
                ...
                <div class="main-content">
                                ...
                                <div class="card">
                                    ...
                                    <div class="card-body p-0">
                                        <div class="list-group list-group-flush">
                                            ...
                                            $stmt = $db->prepare("SELECT DISTINCT u.id, u.name, MAX(a.appointment_date) as last_date 
                                                                FROM appointments a 
                                                                JOIN users u ON a.service_id = u.id 
                                                                WHERE a.user_id = ? AND u.role = 'user'
                    ...
                    <div id="appointments-section" class="content-section">
                        ...
                        <div class="card mb-4">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                ...
                                <div>
                                    <select class="form-select form-select-sm" id="appointment-filter">
                    ...
                    <div id="patients-section" class="content-section">
                        <!-- Nội dung phần bệnh nhân sẽ được thêm vào sau -->
                        <div class="content-header d-flex justify-content-between align-items-center mb-4">
                            <h4 class="mb-0">Danh sách bệnh nhân</h4>
                        </div>
                        <div class="card">
                            <div class="card-body">
                                <p class="text-muted">Nội dung về quản lý bệnh nhân sẽ được cập nhật sau.</p>
                            </div>
                        </div>
                    </div>
    ...
    // Kiểm tra thông báo sau khi reload trang
    ...
    function viewAppointment(id) {
        fetch(`../../controllers/appointment/get.php?id=${id}`)
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="mb-3">
                                        <strong>Ngày khám:</strong> ${new Date(appointment.appointment_date).toLocaleDateString('vi-VN')}
                                    </div>
                                    <div class="mb-3">
                                        <strong>Giờ khám:</strong> ${new Date(appointment.appointment_date).toLocaleTimeString('vi-VN')}
                                    </div>
                                    <div class="mb-3">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <p class="mt-2">${appointment.note || 'Không có ghi chú'}</p>
                                    </div>
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Đóng</button>
                                    ${appointment.status === 'pending' ? `
                                        <button type="button" class="btn btn-success" onclick="updateAppointmentStatus(${id}, 'confirmed')">
                                            Xác nhận
                                        </button>
                                    ` : ''}
                                    ${appointment.status === 'confirmed' ? `
                                        <button type="button" class="btn btn-info" onclick="updateAppointmentStatus(${id}, 'completed')">
                                            Hoàn thành
                                        </button>
                                    ` : ''}
                                </div>
    ...
    function updateAppointmentStatus(id, status) {
        fetch('../../controllers/appointment/update_status.php', {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            if (data.success) {
                ...
                alert(data.message || 'Có lỗi xảy ra khi cập nhật trạng thái lịch hẹn!');
            }
        })
        .catch(error => {
            console.error('Error:', error);
